...
class CK_VISIBLE_PUBLIC CkTask  : public CkMultiByteBase
{
	...
	// When a task is running in a background thread, there are no progress event
	// callbacks. What would normally be a ProgressInfo event callback is instead saved
	// to an in-memory progress log that can be examined and pruned while the task is
	// still running. This property returns the number of progress log entries that are
	// currently available. (Note: the progress log is only kept if the KeepProgressLog
	// property is turned on. By default, the KeepProgressLog is turned off.)
